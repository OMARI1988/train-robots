...
public class TokenizedGenerationContext implements GenerationContext {

	private final Node tokens;

	public TokenizedGenerationContext(String text) {
		tokens = Tokenizer.getTokens(text);
	}

	@Override
	public String getToken(int tokenNumber) {
		return tokens.children.get(tokenNumber - 1).getValue();
	}
}