...
public class DownhillSimplex {
	...
	private static double[] reflect(double[][] simplex, double scale) {
		int i, j, rows = simplex.length, cols = simplex[0].length;
	...
	private static double[][] sort_simplex(double[][] simplex,
			double[] simplex_eval) {
		class IndexComparator implements Comparator<Integer> {
			private double[] m_arr;

			public IndexComparator(double[] arr) {
				m_arr = arr;
			}
		...
		int rows = simplex.length, cols = simplex[0].length;
	...
	double minimize(double[] init, ObjectiveFunction obj_func, Object data) {
		...
		for (i = 0; i < m_maxits
				&& (simplex_eval[numv - 1] - simplex_eval[0]) > TINY; ++i) {
