...
public static PolyMesh pyramid(double w, double h, double d)
  {
    ...
    vtxs[0] = 0.0; vtxs[1] = 0.0; vtxs[2] = 0.0;
    vtxs[3] = w/2; vtxs[4] = h; vtxs[5] = -d/2;
    vtxs[6] = w; vtxs[7] = 0.0; vtxs[8] = 0.0;
    vtxs[9] = w; vtxs[10] = 0.0; vtxs[11] = -d;
    vtxs[12] = 0.0; vtxs[13] = 0.0; vtxs[14] = -d;

    int[] inds = mesh.getFaceInds();
    inds[0] = 0; inds[1] = 2; inds[2] = 1;
    inds[3] = 2; inds[4] = 3; inds[5] = 1;
    inds[6] = 3; inds[7] = 4; inds[8] = 1;
    inds[9] = 4; inds[10] = 0; inds[11] = 1;
    inds[12] = 0; inds[13] = 4; inds[14] = 2;
    inds[15] = 4; inds[16] = 3; inds[17] = 2;
...
public static PolyMesh cube(double w, double h, double d)
  {
    ...
    vtxs[0] = x; vtxs[1] = y; vtxs[2] = z;
    vtxs[3] = x; vtxs[4] = y + h; vtxs[5] = z;
    vtxs[6] = x; vtxs[7] = y + h; vtxs[8] = z - d;
    vtxs[9] = x; vtxs[10] = y; vtxs[11] = z - d;
    vtxs[12] = x + w; vtxs[13] = y; vtxs[14] = z;
    vtxs[15] = x + w; vtxs[16] = y + h; vtxs[17] = z;
    vtxs[18] = x + w; vtxs[19] = y + h; vtxs[20] = z - d;
    vtxs[21] = x + w; vtxs[22] = y; vtxs[23] = z - d;
   
    int[] inds = mesh.getFaceInds();
    
    inds[0] = 0; inds[1] = 1; inds[2] = 2; inds[3] = 3;
    inds[4] = 7; inds[5] = 6; inds[6] = 5; inds[7] = 4;
    inds[8] = 1; inds[9] = 0; inds[10] = 4; inds[11] = 5;
    inds[12] = 3; inds[13] = 2; inds[14] = 6; inds[15] = 7;
    inds[16] = 5; inds[17] = 6; inds[18] = 2; inds[19] = 1;
    inds[20] = 7; inds[21] = 4; inds[22] = 0; inds[23] = 3;
...
public static PolyMesh ellipse(int stacks, int slices,
                               double xs, double ys, double zs)
  {
    int i, j, idx,
        nverts = 2 + ((stacks - 2)*slices),
        ninds = 2*slices + 2*((stacks - 3)*slices);
    double xoff = (double)(2*Math.PI/slices),
          yoff = (double)(Math.PI/(stacks - 1));

    PolyMesh mesh = new PolyMesh(TRI_MESH, nverts, ninds);
    ...
    {
      double ycoord = (double)Math.cos(i*yoff),
            xrad = (double)(Math.sqrt(1.0f - (ycoord*ycoord)));
      
      for(j = 0; j < slices; ++j)
      {
        double angle = (j*xoff);

        vtxs[idx++] = (double)(xs*xrad*Math.cos(angle));
        vtxs[idx++] = ys*ycoord;
        vtxs[idx++] = (double)(zs*xrad*Math.sin(angle));
      }
    }
    ...
    int[] faceinds = mesh.getFaceInds();
    
    idx = 0;
    
    for(i = 0; i < slices; ++i)
    {
      int inds[] = { 0, (i + 1)%slices + 1, i + 1 };

      faceinds[idx++] = inds[0];
      faceinds[idx++] = inds[1];
      faceinds[idx++] = inds[2];
      faceinds[idx++] = nverts - 1;
      faceinds[idx++] = (nverts - 1) - inds[1];
      faceinds[idx++] = (nverts - 1) - inds[2];
    }
...
public static PolyMesh sphere(int stacks, int slices, double s)
  { return ellipse(stacks, slices, s, s, s); }
  ...
  public int getNumVtx() { return m_nvtx; }
  public int getNumPolys() { return m_npolys; }
  public double[] getVertices() { return m_vtxs; }
  public double[] getVertexNormals() { return m_vtxnrms; }
  public double[] getFaceNormals() { return m_facenrms; }
  public int[] getFaceInds() { return m_faceinds; }
  public Material getMaterial() { return m_material; }
  
  public void setMaterial(Material m) { m_material = m; }
  ...
  public void render(GL2 gl, int nrm, boolean use_mtl)
  {
    ...
    if(m_type == TRI_MESH){ gl.glBegin(GL2.GL_TRIANGLES); }
    else if(m_type == QUAD_MESH) { gl.glBegin(GL2.GL_QUADS); }
    else { return; }
    ...
    if(nrm == FACE_NORMALS)
    {
      ...
      {
        idx = i*3;
        gl.glNormal3d(m_facenrms[idx], m_facenrms[idx + 1], m_facenrms[idx + 2]);
    ...
    else
    {
      for(int i = 0, j, idx; i < m_faceinds.length; i += m_type)
      {
        for(j = 0; j < m_type; ++j)
        {
          ...
          gl.glNormal3d(m_vtxnrms[idx], m_vtxnrms[idx + 1], m_vtxnrms[idx + 2]);
          gl.glVertex3d(m_vtxs[idx], m_vtxs[idx + 1], m_vtxs[idx + 2]);
        }
      }
    }
