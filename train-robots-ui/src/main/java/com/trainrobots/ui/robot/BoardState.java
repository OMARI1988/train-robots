...
public static double CELL_OFF = 0.5*(GRID_OFF - OBJ_DIM);
  ...
  private Vec3 m_translate = new Vec3(); // translation of the board from the origin
  private PolyMesh[][][] m_state = new PolyMesh[8][8][8]; // array storing the position of objects in the game
  private PolyMesh m_board = (new OBJReader()).readMesh("Board.obj"); // board mesh
  ...
  { m_translate.x = x; m_translate.y = y; m_translate.z = z; }
  
  public void setArmLocation(int x, int y)
  { m_x = x; m_y = y; }
  
  // gets the coordinate of the centre of a grid cell, used calculating robot arm angles
  public Vec3 getCoord(int x, int y, int z)
  {
    return new Vec3(x*GRID_OFF + 0.5f + m_translate.x + 0.5*OBJ_DIM,
                    (float)(z*OBJ_DIM) + m_translate.y,
                    -y*GRID_OFF - 0.5f + m_translate.z - 0.5*OBJ_DIM);
  }

  // adds a pyramid at the appropriate grid cell
  public void addPyramid(int x, int y, int z,
  ...
  public void moveObj(int sx, int sy, int sz,
                      int dx, int dy, int dz)
  {
    if(m_state[sx][sy][sz] != null &&
       m_state[dx][dy][dz] == null)
    {
  ...
  // release the object at (x, y, z), if z > 0 drops it to the first unoccupied grid location
  ...
  public void render(GL2 gl)
  {
    ...
    gl.glTranslatef((float)m_translate.x, 
                    (float)m_translate.y,
                    (float)m_translate.z);
      ...
      for(j = 0; j < m_state[i].length; ++j)
        for(k = 0; k < m_state[i][j].length; ++k)
        {
          if(m_state[i][j][k] != null)
          {
            ...
            m_state[i][j][k].render(gl, PolyMesh.FACE_NORMALS, true);
            gl.glPopMatrix();
          }
        }
