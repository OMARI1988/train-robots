...
public class Robot implements ObjectiveFunction {
	...
	PolyMesh m_base, m_pivot, m_arm, m_forearm, m_wrist, m_tarsal,
			m_hand_wrist, m_finger1, m_finger2;
	...
	public double getPivotRotY() {
		return m_pivot_roty;
	}

	public double getArmRotZ() {
		return m_arm_rotz;
	}

	public double getForearmRotZ() {
		return m_forearm_rotz;
	}

	public double getWristRotZ() {
		return m_wrist_rotz;
	}

	public double getTarsalRotZ() {
		return m_tarsal_rotz;
	}

	public double getHandRotX() {
		return m_hand_rotx;
	}
	...
	public void setPivotRotY(double ry) {
		m_pivot_roty = ry;
	}

	public void setArmRotZ(double rz) {
		m_arm_rotz = rz;
	}

	public void setForearmRotZ(double rz) {
		m_forearm_rotz = rz;
	}

	public void setWristRotZ(double rz) {
		m_wrist_rotz = rz;
	}

	public void setTarsalRotZ(double rz) {
		m_tarsal_rotz = rz;
	}

	public void setHandRotX(double rx) {
		m_hand_rotx = rx;
	}
	...
	public static Vec3 calcEndEffector(double hand_rotx, double tarsal_rotz,
			double wrist_rotz, double forearm_rotz, double arm_rotz,
			double pivot_roty, double tx, double ty, double tz) {
		double DEG_TO_RAD = 0.01745329;
		Vec3 v = new Vec3(0.0, 0.0, 0.0);
		Mat44 tfrm = Mat44
				.translate(12.0, 0.0, 0.0)
				.mul(Mat44
						.rotateX(DEG_TO_RAD * hand_rotx)
						.mul(Mat44
								.translate(3.0, 0.0, 0)
								.mul(Mat44
										.rotateZ(DEG_TO_RAD * tarsal_rotz)
										.mul(Mat44
												.translate(12.1, 0.0, -2.6)
												.mul(Mat44
														.rotateZ(
																DEG_TO_RAD
																		* wrist_rotz)
	...
	public Vec3 calcEndEffector() {
		return calcEndEffector(m_hand_rotx, m_tarsal_rotz, m_wrist_rotz,
				m_forearm_rotz, m_arm_rotz, m_pivot_roty, m_translate.x,
	...
	public double eval(double[] param, Object data) {
		...
		Mat44 tfrm = Mat44
				.translate(12.1, 0.0, -2.6)
				.mul(Mat44
						.rotateZ(DEG_TO_RAD * param[0])
						.mul(Mat44
								.translate(18.5, 0.0, 1.6)
								.mul(Mat44
										.rotateZ(DEG_TO_RAD * param[1])
										.mul(Mat44
												.translate(25.0, -0.5, -2.0)
												.mul(Mat44
														.rotateZ(
																DEG_TO_RAD
																		* param[2])
		...
		return target.sqdistance(end); // return the square of the distance of
										// the arm end from the target
	}
	...
	public void computeAngles(Vec3 target) {
		...
		tgt.y = Math.max(0.0, tgt.y + 15.0); // offset so that hand doesn't go
												// through board

		double[] param = { m_wrist_rotz, m_forearm_rotz, m_arm_rotz,
				m_pivot_roty };
		...
		m_tarsal_rotz = 0.0;
		Vec3 wrist = (new Vec3()).mul(tfrm), end = calcEndEffector(), dir = (end
				.sub(wrist));
		dir.normalize();
		m_tarsal_rotz = RAD_TO_DEG
				* Math.acos((new Vec3(0.0, -1.0, 0.0)).dot(dir));

		Vec3 rot1 = dir.mul(Mat44.rotateZ(m_tarsal_rotz * DEG_TO_RAD)), rot2 = dir
				.mul(Mat44.rotateZ(-m_tarsal_rotz * DEG_TO_RAD));
